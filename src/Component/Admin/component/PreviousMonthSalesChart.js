...
const PreviousMonthSalesChart = () => {
  ...
  const previousMonth = new Date().getMonth(); // เดือนก่อนหน้า (0-based index)
  const currentYear = new Date().getFullYear(); // ปีปัจจุบัน

  // กรองข้อมูลสำหรับเดือนก่อนหน้าและปีปัจจุบัน
  const filteredSalesData = salesData.filter(
    (item) => item._id.month === previousMonth && item._id.year === currentYear
  );
  ...
  const options = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: "Daily Sales by Category for the Previous Month",
        font: {
          size: 18,
          weight: "bold",
        },
...
export default PreviousMonthSalesChart;
